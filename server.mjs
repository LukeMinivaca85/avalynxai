...
async function handleAPI(req, res, url) {
  ...
  if (url.pathname.startsWith("/api/artifacts")) {
    ...
    await handleArtifacts(req, res, url, body);
...
function serveAssetLinks(req, res) {
  const filePath = join(
    projectRoot,
    "public",
    ".well-known",
...
const server = http.createServer(
  async (req, res) => {
    try {
       ...
       *
       * Precisa responder exatamente:
       * /.well-known/assetlinks.json
       *
       * Essa rota vem antes do fallback da SPA.
       */
      if (
        url.pathname ===
        "/.well-known/assetlinks.json"
      ) {
        serveAssetLinks(req, res);
        return;
      }
...
server.listen(
  ...
  () => {
    console.log(
      `Ava I listening on 0.0.0.0:${port}`
    );

    console.log(
      `NVIDIA NIM configured: ${Boolean(
    ...
    console.log(
      "Android assetlinks route: /.well-known/assetlinks.json"
    );
  }
);
